...
export class ReelGenreComponent implements OnInit, OnDestroy {
  ...
  fetchGenres(search = ''): void {
    ...
    const request$ = search
      ? this.genreService.searchGenre(search)
      : this.genreService.getReelsGenre();

    request$.subscribe({
      next: (res: any) => {
        const list: Genre[] = Array.isArray(res) ? res : (res?.data ?? []);
  ...
  saveGenre(): void {
    ...
    const request$ =
      this.modalMode() === 'edit' && this.editingId !== null
        ? this.genreService.updateGenre(this.editingId, { name })
  ...
  deleteGenre(): void {
    ...
    this.genreService.deleteReelsGenre(this.deleteTargetId).subscribe({
      next: () => {
